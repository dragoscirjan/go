...
```bash
# Clone your fork
git clone https://github.com/yourusername/go.git
cd go
...
- Format: `task format:go` or `gofmt -s -w .`
- Lint: `task lint:go` or `golangci-lint run --fix`
- Check: `task lint:check:go`
...
- Run: `task format` or `npx prettier --write "./**/*.{json,yaml,yml,md}"`
- Check: `task format:check`

### Go Testing

- Run tests: `task test` or `go test -v -race -coverprofile=coverage.out ./...`
- Coverage report: `task test:gen-coverage`
...
# This runs:
# - task format        (Format Go and config files)
# - task clean         (Clean artifacts)
# - task build         (Build Go project)
# - task lint          (Lint all code with golangci-lint and eslint)
